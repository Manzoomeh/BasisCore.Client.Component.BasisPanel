...
export default class WidgetListComponent extends BasisPanelChildComponent {
  ...
  private async saveWidgets(): Promise<void> {
    ...
    widgets.forEach((e: HTMLElement) => {
      ...
      widgetData["x"] =
        e.offsetLeft / cell - Math.floor(e.offsetLeft / cell) > 0.5
          ? Math.floor(e.offsetLeft / cell) + 1
          : Math.floor(e.offsetLeft / cell);
      ...
      if (
        !data.data.find(
          (w) =>
            w.moduleid == widgetData.moduleid &&
            w.widgetid == widgetData.widgetid
        )
      ) {
    ...
    if (sidebar) {
      ...
      widgetData["moduleid"] = allWidgets.find(
        (a) => a.container == "sidebar"
      ).moduleid;
...
`
    ...
    if (this.sortable == null) {
        ...
        if (
          ...
          if (clientX - x > event.source.offsetWidth / 4) {
            ...
            event.sourceContainer
              .querySelectorAll(`.widgetDragClass`)
              .forEach((e: HTMLElement) => {
                if (
                  e.offsetTop === event.source.offsetTop &&
                  e.id != event.source.id
                ) {
            ...
            event.sourceContainer
              ...
              .forEach((e: HTMLElement) => {
                if (
                  e.offsetTop - event.originalSource.offsetTop < 10 &&
                  e.offsetTop - event.originalSource.offsetTop > -10 &&
                  e.id != event.originalSource.id
                ) {
                  e.classList.remove("floatLeft");
                }
              });
      ...
      this.sortable.on("drag:stop", (event) => {
        ...
        if (
          ...
          event.originalSource.setAttribute(
            "gs-h",
            event.source.getAttribute("gs-h")
          );
        }
        if (
          event.source
          ...
          this.disabledDashboardWidgetList =
            this.disabledDashboardWidgetList.filter((e) => {
              return (
                Number(e.widgetid) != Number(widgetData.widgetid) ||
                Number(e.moduleid) != Number(widgetData.moduleid)
              );
            });
          event.originalSource.setAttribute(
            "gs-w",
  ...
  handleMouseMove(ev) {
    ...
    switch (this.type) {
      case "width":
        ...
        if (newWidth != this.currentResizeHandle.offsetWidth) {
  ...
  setTagSelected(e) {
    if (e.target.id !== this.selectedTag) {
      ...
      document
        .querySelector("[data-bc-widget-tag-selected]")
        .removeAttribute("data-bc-widget-tag-selected");
      document
        .querySelector(`[data-bc-widget-tag][id='${this.selectedTag}']`)
...
`
        ...
        const moduleHeader = parent.querySelector(
          `[data-bc-section-header][mid='${widgetList.moduleid}']`
        );
        if (moduleHeader) {
          ...
          const module = this.tags?.find(
            (t) => Number(t.moduleid) === Number(widgetList.moduleid)
          );
          if (module?.title) {
            const headerDiv = document.createElement("div");
            headerDiv.setAttribute("mid", String(module.moduleid));
    ...
    try {
      ...
      if (JSON.parse(data).length) {
        JSON.parse(data).forEach((e) =>
          this.disabledDashboardWidgetList.push(
            ...e.widgets.map((i) => ({ ...i, moduleid: e.moduleid }))
          )
        );
