  ...
  implements IPage
{
  ...
  public addingGroups(pageInfo: IPageInfo):void {
    ...
    pageInfo.groups.forEach((group) => {
      ...
      doc.querySelector('[data-main-print]').setAttribute('datamembername',`${group.groupName}.widgets`)
      doc.querySelector('[data-main-group]').setAttribute('options',`${group.groupName}_options`)
      
      const script = doc.querySelector<HTMLScriptElement>('[data-main-script]')
      script.innerText = `const ${group.groupName}_options = ${`${JSON.stringify(group.options)}`}`;
      const nodes = Array.from(doc.childNodes);
      
      pageBody.appendChild(doc);
      this.owner.processNodesAsync(nodes);
      this.owner.setSource(`${group.groupName}.widgets`,group.widgets);
