...
export default class MenuComponent
    ...
    if (this.toolboxDiv && this.deviceId == 1) {
      ...
      setTimeout(() => {
        ...
        if (Reflect.has(this.owner.manager, 'direction') && toolboxWrapper.innerHTML != "") {
          ...
          else if (Reflect.get(this.owner.manager, 'direction') == "leftToRight") {
            nextSib.style.marginLeft = "0px"
          }
          else{
  ...
  private setMenuUISelected(
    ...
    if (menuItem || pageId == "default") {
      this.menuContainer.querySelectorAll(`[data-bc-menu-active]`).forEach((x) => {
        x.removeAttribute("data-bc-menu-active")
      
        const menuIcon = x.querySelector<HTMLElement>("[data-bc-node-icon-container]")
        const menuIconImgTag = menuIcon.querySelector("img")
        menuIconImgTag.setAttribute("src",menuIconImgTag.getAttribute("data-src"))
      });
      ...
      if (menuItem?.querySelector<HTMLElement>("[data-bc-node-icon-container]")) {
        const menuIcon = menuItem.querySelector<HTMLElement>("[data-bc-node-icon-container]")
        const menuIconImgTag = menuIcon.querySelector("img")
    ...
    if (menuItem && !menuItem.getAttribute("data-bc-bp-menu-toolbox-item")) {
      ...
      if (relatedMenuId) {
        this.menuContainer?.querySelector(`a[data-bc-level="${level}"][data-bc-level-id="${levelId}"][data-bc-mid="${moduleId}"][data-bc-menu-id="${relatedMenuId}"]`)
  ...
  private async onMenuItemClick(
    ...
    this.menuContainer.querySelectorAll('[src="/asset/images/menu_active_circle.png"]').forEach((x) => {
